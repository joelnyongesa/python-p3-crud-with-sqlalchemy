...
if __name__ == '__main__':
    ...
    # Use session maker to configure a 'Session' class
    Session = sessionmaker(bind=engine)
    # Use session class to create a session object
    session = Session()
    ...
    alan_turing = Student(
        name = "Alan Turing",
        email = "alanturing@sherborne.edu",
        grade = 11,
        birthday = datetime(
            year = 1912,
            month = 6,
    ...
    # Order by grade
    students_by_grade = [student for student in session.query(
    ...
    # We can also achieve this using the first() method
    oldest_student_first = session.query(
        Student.name, Student.birthday).order_by(
        desc(Student.grade)).first()

    # print(oldest_student_first)

    # func() gives us access to common SQL operations like sum() and count()
